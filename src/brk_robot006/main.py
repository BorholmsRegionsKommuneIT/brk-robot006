...
if downloadmode == 0:
    bestillingsnavn = user + "_" + "persistent_test_data"
else:
...
def read_single_ansforhold(manummer: str, folder_data_session: Path, bestillingsnavn: str) -> pd.DataFrame:
    ...
    try:
        ...
        nan_columns = [col for col in df_ansforhold.columns if pd.isna(col)]
        df_ansforhold = df_ansforhold.drop(columns=nan_columns)
...
def filter_df_ansforhold(df_ansforhold) -> pd.DataFrame:
    try:
        ...
        def check_if_lonklasse_ineligible(lonklasse: str) -> bool:
            ...
            if parts[0] == 1:
                return 9990 <= parts[1] <= 9999  # noqa: PLR2004
            if parts[0] == 7:  # noqa: PLR2004
                if str(parts[1])[0] == "4":
                    return True
                if str(parts[1])[0] == "9":
                    return True
...
# Funktion der tilføjer kolonner til df_filtered. Gemmer df_filtered under df_over_12_gyldige_ansfh01.
# tjekker om der er mere end 12 gyldige måneder i ansforlob -- #
def over_12_gyldige_ansfh01(df_filtered) -> tuple[int, pd.DataFrame]:
    ...
    df_over_12_gyldige_ansfh01["antal_maaneder"] = (
        df_over_12_gyldige_ansfh01["stopdato_year_month"].astype("int64")
    ...
    return sum_antal_maaneder, df_over_12_gyldige_ansfh01
...
def process_all_ansforhold(df: pd.DataFrame, folder_data_session: Path, bestillingsnavn: str) -> pd.DataFrame:
    ...
    df["maanedsloen_med_pension_fra_dato"] = ""
    ...
    for manummer in manummer_list:
        ...
        # Filter the DataFrame, if it's not empty
        if not df_ansforhold.empty:
            df_filtered = filter_df_ansforhold(df_ansforhold)

        # Sort the DataFrame
        if not df_filtered.empty:
            df_sorted = sort_df_filtered(df_filtered)
            nan_columns = [col for col in df_sorted.columns if pd.isna(col)]
            df_sorted = df_sorted.drop(columns=nan_columns)
            # convert to string
            maanedsloen_med_pension_fra_dato = df_sorted["startdato"].dt.strftime("%Y-%m").to_string(index=False)
            bull = len(df_sorted) == 1
        else:
            maanedsloen_med_pension_fra_dato = ""
            bull = False

        # Update df depending on if df_sorted has exactly one row
        df.loc[df["manummer"] == manummer, "oprettet_pension_maaned"] = bull

        # Update df with the startdato from df_sorted
        df.loc[df["manummer"] == manummer, "maanedsloen_med_pension_fra_dato"] = maanedsloen_med_pension_fra_dato

        # Check if there are more than 12 gyldige ansfh01
        sum_antal_maaneder = over_12_gyldige_ansfh01(df_filtered)[0]
...
def process_all_anshistorik(folder_data_session, df):
    ...
    df["more_than_12_months_ansfh03"] = ""
    df["timeloen_pension_rettidig_dato"] = ""
    ...
    for cpr in df["cprnr"]:
        ...
        if anshistorik is not None and not anshistorik.empty:
            ...
            monthly_hour_threshhold = 34.67  # 34.67

            filtered_anshistorik_grouped = anshistorik_grouped[anshistorik_grouped["antal"] > monthly_hour_threshhold]

            # if filtered_anshistorik_grouped has more than 12 rows, find the 13th year_month in the filtered DataFrame,
            # convert to string and assign in to a variable
            twelve = 12
            if len(filtered_anshistorik_grouped) > twelve:
                timeloen_pension_rettidig_dato = filtered_anshistorik_grouped.iloc[12]["year_month"].strftime("%Y-%m")
            else:
                timeloen_pension_rettidig_dato = ""
            ...
            df.loc[df["cprnr"] == cpr, "timeloen_pension_rettidig_dato"] = timeloen_pension_rettidig_dato
        else:
            df.loc[df["cprnr"] == cpr, "more_than_12_months_ansfh03"] = ""
            df.loc[df["cprnr"] == cpr, "antal_gyldige_maaneder_ansfh03"] = ""
            df.loc[df["cprnr"] == cpr, "timeloen_pension_rettidig_dato"] = ""
...
# ----------------------- Ansforhold output year_month ----------------------- #
"""
Bemærk: Er der udbetalt timeløn på 34,66 timer, eller over,
i samme periode som månedsløn, tælles lønperioden kun én gang.
"""


# Denne funktion findes kun fordi vi skal sammenligne ansforhold og anshistorik.
def stack_year_month_dfs_rem_dup(folder_data_session, bestillingsnavn, df):
    df["ansfh01_og_03_gyldige_maaneder_uden_dub"] = ""

    manummer_list = (df["manummer"]).tolist()
    for manummer in manummer_list:
        # --------------------- Convert ansforhold to year_month --------------------- #
        df_ansforhold = read_single_ansforhold(manummer, folder_data_session, bestillingsnavn)

        # Filter and sort the DataFrame
        df_filtered = filter_df_ansforhold(df_ansforhold)
        df_over_12_gyldige_ansfh01 = over_12_gyldige_ansfh01(df_filtered)[1]

        if not df_over_12_gyldige_ansfh01.empty:
            # select columns 'startdato_year_month', 'stopdato_year_month' from df_ansforhold
            df_over_12_gyldige_ansfh01 = df_over_12_gyldige_ansfh01[["startdato_year_month", "stopdato_year_month"]]

            # Initialize a list to hold the transformed data
            rows = []

            # Iterate over each row in the DataFrame
            for _, row in df_over_12_gyldige_ansfh01.iterrows():
                # Create a range of periods from start to stop date
                period_range = pd.period_range(row["startdato_year_month"], row["stopdato_year_month"], freq="M")

                # Append each period and the corresponding 'manr' to the list
                for period in period_range:
                    rows.append({"year_month": period})

            # Convert the list of dictionaries into a DataFrame
            df_over_12_gyldige_ansfh01 = pd.DataFrame(rows)

            # add an empty column antal to df_over_12_gyldige_ansfh01
            # to be able to stack it with filtered_anshistorik_grouped
            df_over_12_gyldige_ansfh01["antal"] = ""

        # Get filtered_anshistorik_grouped
        cpr = df.loc[df["manummer"] == manummer, "cprnr"].values[0]
        mhtml_path = Path(folder_data_session / f"anshistorik_{cpr}.mhtml")
        anshistorik = parse_ri_html_report_to_dataframe(mhtml_path)
        ...
        if anshistorik is not None and not anshistorik.empty:
            ...
            # bind / stack filtered_anshistorik_grouped to df_over_12_gyldige_ansfh01
            # convert antal to float in filtered_anshistorik_grouped
            filtered_anshistorik_grouped.loc[:, "antal"] = filtered_anshistorik_grouped["antal"].astype(float)

        stacked = pd.concat([df_over_12_gyldige_ansfh01, filtered_anshistorik_grouped], axis=0)
        # remove duplicates friom stacked
        stacked = stacked.drop_duplicates(subset=["year_month"], keep="first")
        stacked_row_count = stacked.shape[0]

        df.loc[df["manummer"] == manummer, "ansfh01_og_03_gyldige_maaneder_uden_dub"] = stacked_row_count

    return df


df = stack_year_month_dfs_rem_dup(folder_data_session, bestillingsnavn, df)


# Save the DataFrame to a CSV file
df.to_csv(path_or_buf=Path(folder_data_session / "df.csv"), index=False, encoding="windows-1252", sep=";")
...
# Hvad hvis noget gå galt hvordan starter vi fra det punk vi slap?

# måske problem: "er medarbejder oprettet med pension på månedsløn" udføres efter alle filtrene er kørt i gennem.

# Lektie til næste robot. Overvej grundigt om kørsel rækkevis eller kolonnevis.
# Processbeskrivelse der både kører rækkevis (personvis) og kolonnevis (funktionsvis).
# Hvad har det af betydning i forhold til at at gentage processen fra et bestemt punkt.
# Lav også en beskrivelse af datastrukturens input og output. Fx tabel.shape
# Alt i alt: beskriv krav og hvor data ligger. Ikke hvodan det skal kodes.
...
logger.info(
    f"the session took {duration} seconds "
    f"with {persistent_df_row_count} medarbejdere "
    f"in downloadmode {downloadmode} on {server_name}"
)
